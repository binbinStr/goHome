...
Page({
    ...
    onLoad: function(u) {
        ...
        app.util.request({
            ...
            success: function(t) {
              console.log( t.data.goods.norms);
              console.log(t.data.goods.norms[0].norms_name);
                var a = t.data.goods.shopprice, e = a, i = t.data.goods, o = void 0, n = p.data.index;
                 if (i.is_norms == 1){
                   o = parseInt(t.data.goods.norms[0].norms_num);
                   p.setData({
                     startbuy: parseInt(t.data.goods.norms[0].norms_num),
                   })
                }else{
                  o = 0 < parseInt(i.startbuy) ? 1 < n ? n : parseInt(i.startbuy) : n
                   p.setData({
                     startbuy: parseInt(t.data.goods.startbuy),
                   })
                }
                p.setData({
                    order: i,
                    ...
                    company: t.data.goods.company,
                    index: o,
                    norms: t.data.goods.norms,
                    height: Math.ceil(t.data.goods.norms.length/4)*55,
                    is_norms: t.data.goods.is_norms,
                    norms_name: t.data.goods.norms[0].norms_name,
                });
            },
        });
    },
  checkboxChange : function(e){
    ...
    for(var i =0;i<norms.length;i++){
      let str = "norms[" + i + "].selected"
        this.setData({
          [str]: false
        })
    }
    let string = "norms[" + e.target.dataset.index + "].selected"
    this.setData({
      [string]: true
    })
    this.setData({
      shopprice: e.target.dataset.value,
      company: e.target.dataset.unit,
      norms_name: e.target.dataset.name,
      startbuy: parseInt(e.target.dataset.num),
  ...
  blurInput:function(e){
    ...
    console.log(i);
    var a = this, n = parseInt(a.data.startbuy);
    if (0 < n && i < n) {
      a.setData({
        index: n
      })
    ...
    addNum: function(t) {
        ...
        // if (100 < i && (i = 99), s) var c = o * i - s; else 
        var c = o * i;
        var d = (o * i).toFixed(2);
        this.setData({
            index: i,
    ...
    reduceNum: function(t) {
        var a = this, e = t.currentTarget.dataset.index, i = parseInt(e) - 1, o = a.data.order, n = parseInt(a.data.startbuy);
    ...
    formSubmit: function(t) {
        ...
        o.setData({
            ...
            if (i = "false", console.log(l + "----" + o.data.startbuy), parseInt(l) < parseInt(o.data.startbuy) && (console.log(11111111111), 
            null != o.data.startbuy && "" != o.data.startbuy && 0 != o.data.startbuy)) return console.log(222222222), 
            void wx.showModal({
                title: "提示",
                content: "该商品起购为" + o.data.startbuy + "个!",
                showCancel: !1
            });
